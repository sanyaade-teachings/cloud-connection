...
const double MessageDisplayDurationSeconds = 5.0;
...
void(*g_UncaughtExceptionCallback)(const char*);
...
- (void)handleException:(NSException *)exception
{
  ...
  if(g_UncaughtExceptionCallback != NULL)
  {
    g_UncaughtExceptionCallback([logErrorDesc UTF8String]);
  }
  ...
  [[UIAlertView alloc]
    ...
    delegate:self
    cancelButtonTitle:[NSString stringWithFormat:@"Quits after %.0lf seconds", MessageDisplayDurationSeconds]
...
void SignalHandler(int signal)
{
  ...
  while(true){} // do not under any circumstances return from the signal handler, because the stack is destroyed
}
...
// registers the exception callback
void SetUncaughtExceptionCallback(void(*callback)(const char*))
{
  g_UncaughtExceptionCallback = callback;
}

/*
 * Havok SDK - Base file, BUILD(#20140514)
 * 
